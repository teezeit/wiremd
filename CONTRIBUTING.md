...
The interactive **review tool** (`pnpm review` from `packages/core/`) renders every fixture with full styles and uses the File System Access API to round-trip status updates through `REVIEW_LOG.md`. **Visual review is enforced**: `tests/review-gate.test.ts` fails on any ⏳ row, so a snapshot change cannot ship until a human has clicked ✅ in the page.
...
pnpm --filter wiremd run review:refresh

# 3. confirm the gate is now red — names every fixture that needs review
pnpm --filter wiremd run test

# 4. eyeball the rendered output, click ✅ on each row that looks right
pnpm --filter wiremd run review

# 5. gate is green again — commit
pnpm --filter wiremd run test && git commit
```

If a fixture is genuinely failing rather than pending review, mark it
with a `<base>.expected-fail.invariants.ts` (sharp contract) or a
`<base>.notes.md` (vague observation), then re-seed: `pnpm review:log`.
...
The wiremd Claude Code plugin lives in `extensions/skills/wireframe/`. It is installed via the skills CLI, the zip upload in Claude Desktop settings, or manually by copying the directory into `.claude/skills/wireframe/`.
...
Users install it once with:

```bash
npx skills add teezeit/wiremd/extensions/skills
```

Or for Claude Desktop (zip upload): download `wireframe-skill.zip` from the latest GitHub release → Settings → Capabilities → Skills → Upload.
