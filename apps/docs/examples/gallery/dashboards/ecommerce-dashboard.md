# E-Commerce Dashboard

> **Use Case:** Online store management dashboard for tracking sales, inventory, and customer data.
>
> **Key Features:** Sales metrics, product inventory, order management, customer insights
::: demo

[[ :logo: ShopDash | *Dashboard* | Products | Orders | Customers | Marketing | Reports | Settings | :bell: (5) | :user: Store Owner ]]

---

## Store Performance

[[ Dashboard > Store Overview ]]

**Date Range:** [Last 30 days...v] [Compare to: Previous period...v] [Export Report]

---
::: columns-4 card
::: column Total Revenue
**$124,567**
:chart: +18.5% vs last period

:::
::: column Orders
**2,456**
:chart: +12.3% vs last period

:::
::: column Avg Order Value
**$50.72**
:chart: +5.2% vs last period

:::
::: column Conversion Rate
**3.2%**
:chart: +0.8% vs last period
:::
:::

---
::: columns-2 card
::: column Revenue Chart (30 Days)

![Line Chart](https://via.placeholder.com/600x300)

**Trends:**
- Highest: $8,234 (Mar 15 - Weekend sale)
- Lowest: $2,456 (Mar 1)
- Average: $4,152/day

[View Detailed Analytics →]

:::
::: column Sales by Category

![Pie Chart](https://via.placeholder.com/600x300)

| Category | Revenue | Orders | % |
|----------|---------|--------|---|
| Electronics | $45,678 | 892 | 37% |
| Clothing | $34,567 | 1,234 | 28% |
| Home & Garden | $23,456 | 456 | 19% |
| Sports | $12,890 | 234 | 10% |
| Other | $7,976 | 140 | 6% |
:::
:::

---

## Recent Orders
::: card

### Latest Transactions

Filter: [All Statuses...v] | Search: [Order # or customer...___________]

| Order # | Customer | Products | Total | Status | Date | Actions |
|---------|----------|----------|-------|--------|------|---------|
| #12456 | John Doe | 3 items | $129.99 | 🟢 Shipped | 10m ago | [View] [Print] |
| #12455 | Jane Smith | 1 item | $89.50 | 🟡 Processing | 1h ago | [View] [Process] |
| #12454 | Bob Wilson | 5 items | $234.99 | 🟢 Shipped | 2h ago | [View] [Track] |
| #12453 | Alice Brown | 2 items | $45.00 | 🔴 Pending | 3h ago | [View] [Confirm] |
| #12452 | Charlie Kim | 4 items | $178.50 | 🟢 Delivered | 5h ago | [View] [Invoice] |

[View All Orders →] [Export to CSV]
:::

---
::: columns-2 card
::: column Low Stock Alerts
::: alert warning
⚠️ **15 products** are running low on stock
:::

| Product | SKU | Current | Min | Status |
|---------|-----|---------|-----|--------|
| Wireless Headphones | WH-001 | 5 | 20 | 🔴 Critical |
| Smart Watch | SW-002 | 12 | 25 | 🟡 Low |
| USB-C Cable | CB-003 | 18 | 50 | 🟡 Low |
| Phone Case | PC-004 | 8 | 30 | 🔴 Critical |
| Power Bank | PB-005 | 15 | 40 | 🟡 Low |

[Restock Now →] [View Full Inventory →]

:::
::: column Top Selling Products

| Product | Sales | Revenue | Stock |
|---------|-------|---------|-------|
| ![IMG] Premium Headphones | 234 | $29,250 | 145 |
| ![IMG] Smart Watch Pro | 189 | $56,700 | 67 |
| ![IMG] Bluetooth Speaker | 156 | $12,480 | 234 |
| ![IMG] Fitness Tracker | 134 | $13,400 | 89 |
| ![IMG] Wireless Earbuds | 128 | $10,240 | 178 |

[View All Products →]
:::
:::

---
::: columns-3 card
::: column Total Customers
**8,234**
+456 new this month

**Breakdown:**
- New: 2,345 (28%)
- Returning: 5,889 (72%)
- VIP: 234 (3%)

:::
::: column Lifetime Value
**$487.23**
Average per customer

**Top Spenders:**
- VIP: $2,345 avg
- Returning: $598 avg
- New: $142 avg

:::
::: column Customer Satisfaction
**4.8** / 5.0
Based on 1,234 reviews

**Rating Distribution:**
- ⭐⭐⭐⭐⭐ 85%
- ⭐⭐⭐⭐ 10%
- ⭐⭐⭐ 3%
- ⭐⭐ 1%
- ⭐ 1%
:::
:::

---

## Order Status Overview
::: columns-5 card
::: column Pending
**45** orders

[View →]

:::
::: column Processing
**123** orders

[View →]

:::
::: column Shipped
**234** orders

[Track →]

:::
::: column Delivered
**1,789** orders

[Archive →]

:::
::: column Cancelled/Returned
**23** orders

[Review →]
:::
:::

---

## Sales by Channel
::: card

### Multi-Channel Performance

| Channel | Orders | Revenue | Conversion | Growth |
|---------|--------|---------|------------|--------|
| Website | 1,234 | $62,456 | 3.2% | +15% ⬆️ |
| Mobile App | 789 | $39,890 | 4.1% | +23% ⬆️ |
| Amazon | 234 | $12,345 | 2.8% | +8% ⬆️ |
| eBay | 123 | $6,789 | 2.1% | -5% ⬇️ |
| Social Media | 76 | $3,087 | 1.9% | +45% ⬆️ |

![Channel Chart](https://via.placeholder.com/700x200)
:::

---
::: columns-2 card
::: column Active Campaigns

**Email Campaign: Spring Sale**
- Sent: 10,000
- Opened: 3,456 (34.6%)
- Clicked: 1,234 (12.3%)
- Converted: 234 (2.3%)
- Revenue: $11,789

[View Campaign →]

**Social Ad: New Arrivals**
- Impressions: 50,000
- Clicks: 2,500 (5% CTR)
- Conversions: 125 (5% CR)
- Spend: $1,250
- Revenue: $6,250
- ROAS: 5.0x

[Manage Ads →]

:::
::: column Discount Codes

| Code | Uses | Revenue | Active |
|------|------|---------|--------|
| SPRING20 | 234 | $11,234 | ✅ Yes |
| FIRST10 | 456 | $4,560 | ✅ Yes |
| VIP25 | 89 | $8,901 | ✅ Yes |
| FREESHIP | 678 | $0 | ✅ Yes |
| EXPIRED | 0 | $0 | ❌ No |

[Create New Code →]
:::
:::

---

## Product Management
::: card

### Quick Actions

[+ Add Product]* [Import Products] [Export Catalog] [Bulk Edit]

**Catalog Stats:**
- Total Products: 789
- Published: 756 (96%)
- Draft: 23 (3%)
- Out of Stock: 10 (1%)

---

### Product Categories

| Category | Products | Sales | Revenue |
|----------|----------|-------|---------|
| Electronics | 234 | 2,345 | $234,567 |
| Clothing | 345 | 5,678 | $123,456 |
| Home & Garden | 123 | 1,234 | $67,890 |
| Sports | 87 | 567 | $34,567 |

[Manage Categories →]
:::

---
::: columns-2 card
::: column Shipping Methods

| Method | Orders | Avg Cost | Delivery |
|--------|--------|----------|----------|
| Standard | 1,234 | $5.99 | 5-7 days |
| Express | 456 | $12.99 | 2-3 days |
| Overnight | 89 | $24.99 | Next day |
| Free Shipping | 677 | $0.00 | 5-7 days |

:::
::: column Fulfillment Stats

**Orders to Ship:** 123
**Average Processing Time:** 1.2 days
**On-Time Delivery Rate:** 96.8%
**Return Rate:** 2.3%

[Manage Shipping →]
:::
:::

---

## Financial Summary
::: card

### Revenue Breakdown (Last 30 Days)

**Gross Revenue:** $124,567
**Discounts:** -$6,234 (5%)
**Refunds:** -$2,456 (2%)
**Net Revenue:** $115,877

**Costs:**
- Product Costs: $62,345 (54%)
- Shipping: $8,901 (8%)
- Marketing: $5,678 (5%)
- Platform Fees: $3,456 (3%)
- Other: $2,345 (2%)

**Net Profit:** $33,152 (29% margin)

![Profit Chart](https://via.placeholder.com/700x200)

[View Financial Reports →]
:::

---

## Customer Support
::: columns-4 card
::: column Open Tickets
**23**
🔴 5 urgent

[View →]

:::
::: column Avg Response
**2.5 hrs**
Target: 4hrs

[Improve →]

:::
::: column Resolution Rate
**94.5%**
First contact

[Details →]

:::
::: column Satisfaction
**4.7** / 5.0
1,234 ratings

[Reviews →]
:::
:::

---

## Recent Reviews

| Product | Customer | Rating | Review | Date |
|---------|----------|--------|--------|------|
| Wireless Headphones | John D. | ⭐⭐⭐⭐⭐ | "Amazing sound quality!" | 1h ago |
| Smart Watch | Sarah L. | ⭐⭐⭐⭐ | "Great features, battery could be better" | 3h ago |
| Bluetooth Speaker | Mike C. | ⭐⭐⭐⭐⭐ | "Perfect for outdoor use" | 5h ago |

[Moderate Reviews →] [Reply to Customers →]

---

## Tasks & Reminders
::: card

### Today's Tasks

- [x] Process pending orders (12/12 complete)
- [x] Respond to customer inquiries (5/5 complete)
- [ ] Restock low inventory items (0/15 complete)
- [ ] Review marketing campaign performance
- [ ] Update product descriptions

**Upcoming:**
- Weekly inventory audit (Tomorrow)
- Monthly sales report (3 days)
- Supplier payment due (5 days)

[Manage Tasks →]
:::

---
::: footer
**Last Updated:** 2 minutes ago • [Refresh Data] • [Download Report] • [Help Center] • [Contact Support]
:::
:::

---

**Style Variations:**
- `sketch` - Quick e-commerce prototype
- `clean` - Professional shop dashboard
- `wireframe` - Data structure focus
- `tailwind` - Modern retail interface
- `material` - Google Merchant style
- `brutal` - Bold commerce UI
- `none` - Semantic HTML for custom styling

**Generate this example:**
```bash
wiremd ecommerce-dashboard.md --style sketch
wiremd ecommerce-dashboard.md --style clean -o ecommerce-clean.html
wiremd ecommerce-dashboard.md --style tailwind -o ecommerce-tailwind.html
```
