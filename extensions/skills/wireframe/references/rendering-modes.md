...
|---|---|---|
| Write files **and** run shell | **Files in folder** *(default)* — render `.html` into the user's folder; they open via `file://` or double-click | Single page: `wiremd x.md -o x.html --style sketch`. Multi-page: `wiremd wireframes/ --style sketch` (+ `.md`→`.html` sed rewrite; see `multi-page.md`) |
| Write + shell, **and user runs Claude locally** on their own machine | **Dev server** — live reload while iterating. Only works when the user can reach `localhost` on the Claude host. | Single page: `wiremd x.md --style sketch --serve 3001 --watch`. Multi-page: `wiremd wireframes/ --style sketch --serve 3001 --watch` |
| Write + shell, **reviewer is non-technical (no VS Code)** | **Browser editor with `?file=` hint** — write the `.md`, generate a hint URL, share it. Reviewer links the file; Claude's edits auto-refresh within ~500ms. Requires Chrome/Edge/Safari 16.4+. | `node -e "const u=new URL('https://teezeit.github.io/wiremd/editor/');u.searchParams.set('file',process.argv[1]);console.log(u.toString())" /path/to/file.md` |
...
| Chat only (no filesystem, no exec) | **Hand off to the web editor** — Claude writes the `.md`; user pastes it into the hosted editor for rendering | Share `https://teezeit.github.io/wiremd/editor` + the `.md` content |

## Browser editor with `?file=` hint

The live editor at `https://teezeit.github.io/wiremd/editor/` supports bidirectional file sync via the browser's File System Access API. Use this when the reviewer has no VS Code but can open a URL in Chrome, Edge, or Safari 16.4+.
...
2. Claude generates a hint URL and hands it to the reviewer:
   ```bash
   node -e "const u=new URL('https://teezeit.github.io/wiremd/editor/');u.searchParams.set('file',process.argv[1]);console.log(u.toString())" /path/to/wireframe.md
   ```
