...
**5. Badge syntax in table cells.** Closed: `((cell)){.warning}` works inside `<td>` without escaping table pipes. Legacy escaped `\|cell\|{.warning}` remains covered as an alias.
...
- **Badge syntax migration** — `((pill content))` is the primary badge syntax. Legacy `|content|{.modifier}` is still accepted as an alias during migration.
